...
shared_ptr<Material> MaterialFactory::createMaterial(vec3 a, vec3 d, vec3 s, float beta, vec3 kt, float ui, float opacity, MATERIAL_TYPES t){
    shared_ptr<Material> m;
    switch (t) {
    case LAMBERTIAN:
        m = make_shared<Lambertian>(a, d, s, beta, opacity);
    ...
    case METAL:
        ...
        break;
    case TRANSPARENT:
        m = make_shared<Transparent>(a, d, s, beta, kt, ui, opacity);
        break;
    case MATERIALTEXTURA:
        m = make_shared<MaterialTextura>(a, d, s, beta, opacity);
        break;
