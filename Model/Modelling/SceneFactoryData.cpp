...
shared_ptr<Object> SceneFactoryData::objectMaps(int i,int j) {
    ...
    //Obtenemos los rangos de las coordenadas x y z del mundo real y virtaul

    float RXrange = mapping->Rxmax - mapping->Rxmin;
    float RZrange = mapping->Rzmax - mapping->Rzmin;
    float VXrange = (mapping->Vxmax - mapping->Vxmin) ;
    float VZrange = (mapping->Vzmax - mapping->Vzmin) ;

    vec3 dataPos = dades[i].second[j];
    // Normalizamos las coordenadas x y z de los datos, con el rango mínimo y máximo en el mundo real. [0, 1]
    float normalizedX = (dataPos.x - mapping->Rxmin) / RXrange;
    float normalizedZ = (dataPos.y - mapping->Rzmin) / RZrange;

    // Escala las coordenadas normalizadas al rango mínimo y máximo en el espacio virtual (interpolacion lineal)
     float virtualX = mapping->Vxmin + (normalizedX * VXrange);
      float virtualZ = mapping->Vzmin + (normalizedZ * VZrange);

     vec3 trans(virtualX, 0, -virtualZ);

    // b. Calcula la translació

     o->aplicaTG(make_shared<TranslateTG>(trans));

     // Nos aseguramos que la componente z de data este entre el valor minimo y maximo  del gyzmo.
     float infoData = dataPos.z;
     float minValue = mapping->attributeMapping[i]->minValue;
     float maxValue = mapping->attributeMapping[i]->maxValue;
     float rangeData = maxValue - minValue;
     float VYrange = (mapping->Vymax - mapping->Vymin)*0.5;


     float normalizedData = (infoData - minValue) / rangeData;

     // Obtenemos la escala en función del rango de valores en el mundo virtual de y.
     float scaleFactor = (normalizedData * VYrange) + 0.1;

     // c. Aplica la TG a l'objecte usant
      o->aplicaTG(make_shared<ScaleTG>(vec3(scaleFactor, scaleFactor, scaleFactor)));
      return o;
