...
void Sphere::aplicaTG(shared_ptr<TG> t) {
    if (dynamic_pointer_cast<TranslateTG>(t)) {
        ...
        center.y = c.y;
        center.z = c.z;
        QTextStream(stdout) << "Modifica el centro esfera despues de la translacion " << center[0] << ", "<< center[1] << ", "<< center[2] << "\n";
    }
    //TODO: Cal ampliar-lo per a acceptar Escalats
    else if (dynamic_pointer_cast<ScaleTG>(t)) {
            // Es realitzen els escalats
             vec4 r(radius,1.0,1.0,1.0);
             r = t->getTG() * r;
             radius = r.x;
             QTextStream(stdout) << "R esfera despues del escalado:: " << radius<< "\n";
