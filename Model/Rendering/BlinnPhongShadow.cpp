...
vec3 BlinnPhongShadow::shading(shared_ptr<Scene> scene, HitInfo& info, vec3 lookFrom, vec3 globalLight, vector<shared_ptr<Light>> lights)
{
    ...
    for (auto light : lights) {
        ...
        if (dot(L, N) > 0.0f) {
            ...
            color *= computeShadow(light, p + (epsilon*L), scene);
            color += Ia * ka;
        }
        colorShadow += color;
        color= vec3(0,0,0);
